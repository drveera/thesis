...
To analyze the performance of TDMA a series of network performance tests were run using Netperf, an open source network performance tool that is installed by default on most Linux distributions~\cite{netperf}. These tests were run on a TDMA testbed run in Impact Lab at Arizona State University. The results showed a small, but significant overhead in terms of reduced throughput when using TDMA to separate network traffic. The results also clearly show the temporal division of network traffic between security groups. 
...
Using the TDMA testbed we ran two series of performance tests, one to display the temporal division of security group packets, and the other to measure TDMA's overhead on system throughput.
\subsection{Netperf}
